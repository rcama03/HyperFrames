...
ZOOM_TIMES  = [0.0, 38.34, 76.69, 115.03, 153.38, 191.72, 230.06, 268.41, 306.75, 345.10]

# Stat card inTimes → screen shake
SHAKE_TIMES = [46, 84, 199, 238, 355, 465]

# Swoosh on every card appearance
SWOOSH_CARD_TIMES = [
    0.0, 38.34, 76.69, 115.03, 153.38, 191.72, 230.06, 268.41, 306.75, 345.10,  # chapters
    46, 84, 199, 238, 355, 465,                                                     # stats
    94, 163, 316, 425,                                                               # alerts
    103, 128, 178, 283, 505,                                                         # keys
    160, 445,                                                                        # definitions
    370,                                                                             # quote
    50, 243, 490,                                                                    # sources
    387,                                                                             # rank
...
# ── ASS caption builder — scaled for 720p ────────────────────────────────────
def _ts_ass(t):
...
# ── Build & run ffmpeg ────────────────────────────────────────────────────────
def main():
    SRT_SRC = '/root/.claude/uploads/bf195af2-7fab-5825-a36f-49fc2b3be335/e21bfe51-captions.srt'
    ...
    # 5. Zoom punch-ins on chapter cards
    ZW = int(W * ZOOM_SCALE)
    ZH = int(H * ZOOM_SCALE)
    ZX = (ZW - W) // 2
    ZY = (ZH - H) // 2
