...
# ── Helper: slide-in expression (0.5s ease, stays until outTime) ─────────────
SLIDE_DUR = 0.5

def slide_expr(card):
    T   = card["inTime"]
    T05 = T + SLIDE_DUR
    entry = card.get("entry", "right")
    if entry == "right":
        x = f"if(gte(t,{T}),if(lt(t,{T05}),{WIDTH}-{WIDTH}*((t-{T})/{SLIDE_DUR}),0),{WIDTH})"
        y = "0"
    elif entry == "left":
        x = f"if(gte(t,{T}),if(lt(t,{T05}),-{WIDTH}+{WIDTH}*((t-{T})/{SLIDE_DUR}),0),-{WIDTH})"
        y = "0"
        ...
        x = "0"
        y = f"if(gte(t,{T}),if(lt(t,{T05}),{HEIGHT}-{HEIGHT}*((t-{T})/{SLIDE_DUR}),0),{HEIGHT})"
    elif entry == "top":
        x = "0"
        y = f"if(gte(t,{T}),if(lt(t,{T05}),-{HEIGHT}+{HEIGHT}*((t-{T})/{SLIDE_DUR}),0),-{HEIGHT})"
    else:
...
for idx, card in enumerate(cards):
    ...
    x, y = slide_expr(card)
    vf.append(
        f"{current}[{card_stream}:v]overlay=x='{x}':y='{y}':"
...
print(f"\nRunning FFmpeg ({n_cards} cards, {DURATION:.1f}s)...\nOutput → {OUT_VIDEO}\n")
