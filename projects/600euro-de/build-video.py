#!/usr/bin/env python3
"""
Assembles the 600euro-de video using FFmpeg.
...
SRC_VIDEO  = sys.argv[1] if len(sys.argv) > 1 else str(HERE / "source-video-new.mp4")
...
ZOOM_TIMES  = [0.5, 68.5, 136.0, 201.5]
SHAKE_TIMES = [21.0, 44.0, 192.0]
...
# Scale card times if timestamps were scaled
if abs(timing_end - VOICE_DUR) > 0.5:
    scale = VOICE_DUR / timing_end
    for c in cards:
...
# Pad video if shorter than voiceover (freeze last frame)
...
# Original video audio (transitions) — pad if needed
if VIDEO_DUR < DURATION:
...
# Voiceover sidechain ducking on music
...
af.append(
    ...
    f"amix=inputs={n_mix}:duration=first:weights=1 1 0.7"
    + " 0.8" * n_cards
    + "[audio_out]"
)
