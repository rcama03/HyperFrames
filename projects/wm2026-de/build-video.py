#!/usr/bin/env python3
"""
Assembles the wm2026-de video using FFmpeg.
WM 2026 Flug USA buchen - die wichtigsten Tipps jetzt
Video is looped to match voiceover duration (522.72s video vs 580.128s voice).
"""
import json
import subprocess
import sys
...
ZOOM_TIMES  = [94.8, 181.8, 273.4, 361.4, 449.0, 544.8]
# Dramatic moments: hook stat + Hopper study + Währung chapter
SHAKE_TIMES = [2.5, 245.8, 449.0]
...
# Use voice duration as output length; video is looped to fill it
DURATION     = VOICE_DUR

_fr = video_stream.get("r_frame_rate", "25/1").split("/")
FPS = int(_fr[0]) // int(_fr[1])

print(f"Source : {Path(SRC_VIDEO).name}  {WIDTH}x{HEIGHT}  {VIDEO_DUR:.1f}s  {FPS}fps (looped to {DURATION:.1f}s)")
...
# -stream_loop -1 loops the source video to cover the full voice duration
inputs = ["-stream_loop", "-1", "-ss", str(SRC_OFFSET), "-i", SRC_VIDEO, "-i", VOICE_MP3]
for c in cards:
    inputs += ["-i", c["path"]]
music_idx  = 2 + n_cards
