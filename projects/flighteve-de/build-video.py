#!/usr/bin/env python3
"""
Assembles the flighteve-de video using FFmpeg.
...
Usage:
  python3 build-video.py [source-video.mp4] [voiceover.mp3]
"""
import json
import subprocess
import sys
...
SHAKE_DUR    = 0.7
BAR_H        = 8
BAR_COLOR    = "0xFFB300"  # amber

SRC_OFFSET = 0.0

# Chapter card inTimes (trigger zoom)
ZOOM_TIMES  = [0.5, 101.0, 201.5, 281.0, 378.0, 473.0]

# Dramatic stat reveals (trigger shake)
SHAKE_TIMES = [22.0, 108.0, 368.0]
...
print(f"Source : {Path(SRC_VIDEO).name}  {WIDTH}×{HEIGHT}  {VIDEO_DUR:.1f}s  {FPS}fps")
...
#   [N+3]     swoosh
#   [N+4]     lavfi color source for progress bar
...
# ── Amber progress bar (scale+overlay, eval=frame) ────────────────────────────
