...
ZOOM_TIMES  = [0.0, 46.0, 90.0, 140.0, 179.0, 226.0, 270.0, 313.0, 363.0, 406.0, 429.0]

SHAKE_TIMES = [8.0, 65.0, 98.0, 157.0, 237.0, 440.0]

SWOOSH_CARD_TIMES = [
    0.0, 8.0, 20.0, 46.0, 55.0, 65.0, 67.0, 90.0, 98.0, 112.0,
    140.0, 148.0, 157.0, 179.0, 185.0, 200.0, 226.0, 237.0, 270.0, 283.0,
    313.0, 330.0, 332.0, 363.0, 375.0, 393.0, 406.0, 415.0, 429.0, 440.0,
    442.0, 480.0, 505.0, 525.0, 548.0,
]
...
def srt_to_words(path):
    ...
    for b in blocks:
        ...
        if len(lines) < 3:
            continue
        times = lines[1].split(' --> ')
        start = _srt_ts(times[0].strip())
        end   = _srt_ts(times[1].strip())
...
def main():
    SRT_SRC = '/root/.claude/uploads/bf195af2-7fab-5825-a36f-49fc2b3be335/1d953449-captions.srt'
